...
import { addBlog, finishEditing,cancelEditing } from '../../blogSlice'
...
export default function CreateBlog({ onClose, activeModal }: BlogFormProps) {
  const [formData, setFormData] = useState<BlogType>(initialForm)
  ...
  if (editingBlog) {
    setFormData(prev =>
      prev.id === editingBlog.id ? prev : editingBlog
    )
  } else {
  ...
  return (
    <div className='modal-root fixed inset-0 z-50 flex items-center justify-center'>
      <div
        className='overlay absolute inset-0 bg-black/80 backdrop-blur-sm'
        onClick={onClose}
      ></div>
